...
export default function ProviderDealsView({ }) {
  ...
  if (dealsSuccess && minerSuccess) {
    myDeals = deals.map((deal) => ({ ...deal })).filter((deal) => deal.provider_actor_id.toString() === minerId.toString());
  }
  console.log('PROVIDER DEALS');
  console.log(myDeals);
  return (
    <Box
      element="div"
